-- =========================================================================
-- KoshAgar ERP - 45_purchase_cash_guard.sql
--
-- A PURCHASE CANNOT SPEND MONEY THE DRAWER DOES NOT HAVE
--
-- post_cash_withdrawal has refused to overdraw since 08_cash_documents.sql:
-- "a drawer cannot hold less than nothing". post_purchase never made the same
-- check, so cash could leave for goods even when there was none to leave.
--
-- The app does ask - it opens a funding dialog and records an investment or a
-- loan first - but that question is asked by the BROWSER. A rule that lives
-- only on a screen is missing everywhere else the same write can happen: an
-- edit, a retry after a failure, an import, a future screen written by someone
-- who never saw the dialog. The database is the only place a rule cannot be
-- walked around.
--
-- This is the same function as 28_extra_cost_is_cash.sql with that one check
-- added; nothing else is changed.
--
-- KNOWN CONSEQUENCE, ACCEPTED
-- A backdated purchase into a day whose drawer was already short will now be
-- refused rather than silently deepening it. That is the point of the rule, and
-- the message names the day and the amount so the shortfall can be recorded as
-- what it was - an investment, or a loan.
-- =========================================================================
...
begin
  ...
  if v_cash_paid + v_extra > 0 then
    ...
    if v_cash_paid + v_extra > cash_balance_as_of(p_shop, v_account, v_date) + 0.001 then
      raise exception
...
select 'post_purchase() now refuses to overdraw the drawer' as note;
